...
import numpy as np
import pandas as pd
#vcf_file = allel.read_vcf("/home/ubuntu/emiliano/2nd_chp/gatk_analyses/annotate_VCF/with_NewAnnotate/$j/mac_2/4fold_mac2_$j.vcf.gz", fields='*', region='$i')
...
pi, windows, n_bases, counts = allel.windowed_diversity(pos_3, final_allele_counts, size = 50000, is_accessible = acc_sites, start = 1, stop = pos_$i)
pd.set_option('display.float_format', '{:.20f}'.format)
pi_df = pd.DataFrame({'C1':pi})
pd.set_option('display.float_format', '{:.8f}'.format)
pi_df_round = pi_df.round(8)
windows_df = pd.DataFrame({'C1':windows[:,0], 'C2':windows[:,1]})
n_bases_df = pd.DataFrame({'C1':n_bases})
counts_df = pd.DataFrame({'C1':counts})
path = '/home/ubuntu/emiliano/2nd_chp/scikit/pi/windows'
sys_stdout = open(path, 'w')
sys_stdout.write(windows_df.to_string())
sys_stdout.close()
path = '/home/ubuntu/emiliano/2nd_chp/scikit/pi/n_bases'
sys_stdout = open(path, 'w')
sys_stdout.write(n_bases_df.to_string())
sys_stdout.close()
path = '/home/ubuntu/emiliano/2nd_chp/scikit/pi/counts'
sys_stdout = open(path, 'w')
sys_stdout.write(counts_df.to_string())
sys_stdout.close()
path = '/home/ubuntu/emiliano/2nd_chp/scikit/pi/ave_pi'
sys_stdout = open(path, 'w')
sys_stdout.write(pi_df_round.to_string())
sys_stdout.close()
END

paste ave_pi windows counts | tail -n+2 | awk '{print $2,$4,$5,$7}' | sed 's/$/\t'$i'/' | awk '{print $0,NR}' | awk -v OFS='\t' '{print $5,$6,$1,$2,$3,$4}' > $i
rm ave_pi counts n_bases windows; done
cat ${chr[@]} >> ''$j'_4fold_pi'
rm ${chr[@]}; done
...
import numpy as np
import pandas as pd
#vcf_file = allel.read_vcf("/home/ubuntu/emiliano/2nd_chp/gatk_analyses/annotate_VCF/with_NewAnnotate/$j/mac_2/0fold_$j_mac2.vcf.gz", fields='*', region='$i') # 0-fold estimates
...
path = '/home/ubuntu/emiliano/2nd_chp/scikit/pi/ave_pi'
sys_stdout = open(path, 'w')
sys_stdout.write(pi_df.to_string())
sys_stdout.close()
END
